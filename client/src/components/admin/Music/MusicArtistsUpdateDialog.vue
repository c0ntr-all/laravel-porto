...
interface UpdateArtistApiResponse {
  data: ResponseArtist
  included: any
  meta: {
    message: string
...
const emit = defineEmits<{
  ...
  (e: 'updated', value: Artist): void
}>()

const model = ref<ArtistModel>({
  name: props.artist.name,
  image: props.artist.image,
  description: props.artist.description,
  tags: {
    base: props.artist.relationships.tags.data.filter((tag: Tag) => tag.is_base),
    secondary: props.artist.relationships.tags.data.filter((tag: Tag) => !tag.is_base)
  }
})
...
const baseOptions = ref<Tag[]>([])
const secondaryOptions = ref<Tag[]>([])
const newImage = ref()
const show = ref(props.modelValue)
const updateButtonLoading = ref(false)

const transformArtistFromResponse = inject<((responseArtist: ResponseArtist, responseData: any) => Artist)>('transformArtistFromResponse')!
...
const updateArtist = async () => {
  ...
  await api.post<UpdateArtistApiResponse>(`v1/music/artists/${props.artist.id}`, formData)
    .then(response => {
...
const getTagsSelect = async () => {
  await api.get('v1/music/tags')
    .then(response => {
      const allTags = response.data.data.map((responseTag: ResponseTag) => {
        return {
          id: responseTag.id,
      ...
      baseTags.value = baseOptions.value = allTags.filter((tag: Tag) => tag.is_base)
      secondaryTags.value = secondaryOptions.value = allTags.filter((tag: Tag) => !tag.is_base)
